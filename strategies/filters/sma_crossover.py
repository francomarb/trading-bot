...
SMAEdgeFilter gates new entries on three conditions:
  ...
  2. Stock structural strength — stock close > stock 200-day SMA.
  ...
  3. Volume expansion — 10-day average volume > 30-day average volume.
...
class SMAEdgeFilter:
    """
    Entry gate for SMA Crossover: SPY > 200SMA AND stock > 200SMA AND volume expanding.
    ...
    def __call__(self, df: pd.DataFrame) -> pd.Series:
        ...
        combined = spy_gate & stock_gate & vol_gate
        ...
        if not df.empty:
            ...
            if allowed:
                logger.debug(
                    ...
                    f"SPY>200SMA stock>200SMA vol_expanding"
                )
            else:
                reasons = []
                if not spy_ok:
