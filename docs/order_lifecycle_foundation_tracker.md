...
- ~~**Recovered-entry accounting completeness**~~ ✅ CLOSED. `_maybe_dispatch_substrate_entry_fill` now replays `_log_entry` after `_apply_recovered_entry_side_effects` succeeds. The substrate's `apply_order_event` UPSERT writes the trade row with provenance fields preserved at submit (benchmark price / kind / timestamp / quality), but leaves `slippage_signed_bps` and `slippage_adverse_bps` as NULL because that math lives in `build_record`. The follow-up dispatch pulls the substrate row's `slippage_benchmark_price` as `modeled_price` and calls `_log_entry`, which UPSERTs again: `tl.log`'s PRESERVE-FIRST-NON-NULL policy keeps the substrate's submit-time provenance while the LATEST-NON-NULL policy fills in the computed signed/adverse columns. Integration tests `TestEntryDispatchSlippageCompleteness` (mirroring `TestExitDispatchEndToEnd`) pin both the populated-benchmark path (signed/adverse computed) and the missing-benchmark path (honest NULL — no fabricated benchmark). PR #61 round-2 reviewer.
