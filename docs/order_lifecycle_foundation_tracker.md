...
- **2026-09-22 NULL-order_id sweep observability correction:** live paper evidence showed that a transport exception before broker-id attachment can leave both a durable pending row and a transient stream watch. The sweep correctly resolved the row from Alpaca, but its pre-lookup stale alert falsely claimed the row was not self-healing, and the resolved watch continued making client-id gap lookups. The sweep now emits the one-hour CRITICAL only when a broker lookup or durable transition leaves the row unresolved. Successful attachment and broker-authoritative 404 rejection release the transient watch; unresolved rows retain it. This supersedes the earlier “stale threshold always emits CRITICAL” wording above without changing submission, reconciliation, or trading behavior.
