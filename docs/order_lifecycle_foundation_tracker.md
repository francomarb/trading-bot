...
- **2026-09-22 NULL-order_id sweep observability correction:** live paper evidence showed that a transport exception before broker-id attachment can leave both a durable pending row and a transient stream watch. The sweep correctly resolved the row from Alpaca, but its pre-lookup stale alert falsely claimed the row was not self-healing, and the rejected order's watch continued making client-id gap lookups. The sweep now emits the one-hour CRITICAL only when a broker lookup or durable transition leaves the row unresolved. Broker-authoritative 404 rejection and terminal attachment release the transient watch; alive attached orders retain it for the submitting waiter and reconnect recovery. This supersedes the earlier “stale threshold always emits CRITICAL” wording above without changing submission, reconciliation, or trading behavior.

- **Historical exit-substrate note:** the June 2026 sample had no `role='exit'` rows because its option closes were protective stops. The later exit-substrate validation is complete. The related underlying-vs-OCC ownership ambiguity is also closed: single-leg options now use lifecycle-UID identity and exact OCC symbols throughout close detection and accounting.
