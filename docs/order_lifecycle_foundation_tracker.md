...
- ~~**NULL-order_id attach orphaning**~~ ✅ CLOSED via the NULL-order_id REST sweep on `feat/null-order-id-rest-sweep` (7 commits: store query + `mark_pending_unknown_to_broker` + engine `_sweep_null_order_id_attaches` + startup/cycle wiring + 12 acceptance tests + docs). Cycle sweep is bounded at 5 REST calls (additive to `_SUBSTRATE_CYCLE_RECONCILE_LIMIT=20`, different endpoint/recovery target); startup is unbounded (one-shot backlog drain). The sweep walks `position_lifecycle_orders` rows where `order_id IS NULL AND status='pending' AND created_at <= now - 60s`, filtered to `position_type='single_leg'` and `role != 'partial_close'`. Three outcomes per row: (a) broker returns alive order → `attach_broker_order_id` so the regular reconciler picks it up; (b) broker returns terminal order → attach + `apply_order_event` + dispatch helpers in one pass; (c) broker returns 404 → `mark_pending_unknown_to_broker` so the position-status CTE walks the parent out of pending. Composition with PR #71's trailing-stop `client_order_id` fallback: both call `attach_broker_order_id`; the racer's matching attach is treated as benign via a ValueError catch + re-check (no double-attach, no false CRITICAL). Stale-orphan threshold at 1h emits CRITICAL so an operator sees real anomalies. The CRITICAL log in `_drain_lifecycle_attaches` (engine/trader.py:7027) STAYS as the real-time orphan signal even after the sweep ships — the pair (initial-attach CRITICAL → next-cycle sweep INFO recovery) is itself diagnostic evidence the substrate path is healthy. **Evidence-cold**: zero `SUBSTRATE ROW ORPHANED` messages have ever fired across the paper log archive at the time of cut. PR #61 round-2 reviewer (originally raised). _Historical context below preserved for follow-up readers._ The failure mode: cycle and startup reconcilers walk rows with `order_id IS NOT NULL` only. A synchronous attach failure OR a bot crash between async submit and the next cycle's drain leaves the row at `pending` with `order_id=NULL`. The lifecycle-attach queue is in-memory and lost on restart — it does NOT re-drain. The substrate row is then ORPHANED until manually resolved. The CRITICAL log in `_drain_lifecycle_attaches` (engine/trader.py:4568) says exactly this so the operator knows to inspect. Recovery requires a separate REST path that walks NULL-order_id rows and resolves via `get_order_by_client_id`. Uncommon trigger; not blocking on paper, but the failure mode is more serious than the earlier "self-heals on restart" framing implied. PR #61 round-2 reviewer. **Partial mitigation for options trailing stops shipped in PR #71 (§10.4)**: `_lifecycle_order_id_for` now accepts a `client_order_id` fallback, looks up by `client_order_id` when the `order_id` lookup misses, and opportunistically re-attaches the broker `order_id` on a hit — so the `attach_broker_order_id`-failed-but-`insert_pending`-succeeded variant self-heals on the next cycle for the options trailing path. The general equity / cross-role NULL-order_id-by-client_order_id REST sweep remains the proper fix and is unaffected by PR #71. **MLEG side no longer in scope of this follow-up after §10.7 ships**: the spread close substrate path inserts the pending row *before* `dispatch_spread_order` returns (so order_id starts NULL by design), and the worker drain attaches the broker order_id at terminal-state advance via `mark_terminal_after_dispatch`. The partial_close residual placeholder is *intentionally* order_id=NULL — the substrate reconciler skips it and the next dispatch is what advances the row (or operator cancels it). Neither shape requires the general NULL-order_id REST sweep.
