...
- ~~**Recovered-entry accounting completeness**~~ ✅ CLOSED via PR #68. `_maybe_dispatch_substrate_entry_fill` now (a) accepts `partially_filled` in addition to `filled`, (b) builds `build_record` + `trade_logger.log` inline after `_apply_recovered_entry_side_effects` succeeds so identity-conflict failures reach the granular CRITICAL log instead of getting absorbed at error inside `_log_entry`, and (c) mirrors the event status into `OrderResult` so the trade row's status doesn't get clobbered from `'partially_filled'` to `'filled'`. The substrate's `apply_order_event` UPSERT writes the trade row with provenance fields preserved at submit (benchmark price / kind / timestamp / quality); the dispatch follow-up pulls the substrate row's `slippage_benchmark_price` as `modeled_price` and runs the trade-log write, where `tl.log`'s PRESERVE-FIRST-NON-NULL policy keeps the substrate's submit-time provenance and the LATEST-NON-NULL policy fills in the computed signed/adverse columns. The engine submit path was also fixed to forward order-type-aware tags (`'limit_price'`/`'unavailable'` for LIMIT and STOP_LIMIT) so the substrate's submit-time tag isn't `'arrival_midpoint'`/`'primary'` on passive entries, which otherwise would have survived the PRESERVE-FIRST-NON-NULL COALESCE and persisted the wrong quality forever — exactly the dashboard/health/calibration smell PR #67 set out to eliminate. Integration tests `TestEntryDispatchSlippageCompleteness` (six tests mirroring `TestExitDispatchEndToEnd`) pin: populated-benchmark MARKET path (signed/adverse computed), missing-benchmark path (honest NULL), LIMIT recovery preserving `'limit_price'`/`'unavailable'`, single partial-fill dispatching with computed slippage on the partial price, partial-then-filled single-shot via ownership gate, and the completeness-call-failure CRITICAL handler. Engine submit-time tagging pinned by `TestOrderTypeAwareSubmitSlippageTagging` (three tests covering MARKET-with-quote, MARKET-fallback-to-close, LIMIT-forced-unavailable). PR #61 round-2 reviewer.
