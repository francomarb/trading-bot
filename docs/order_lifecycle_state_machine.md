...
1. ~~**Stream-driven entry fill for equities.**~~ **NOW WIRED — corrected 2026-08-14.** This entry claimed a stream ENTRY fill had no path to `mark_open`. That is no longer true: `_drain_lifecycle_events` applies every state-bearing stream event through `apply_order_event`, whose step 4 runs `_POSITION_STATUS_SQL` — a CTE deriving position status from the order rows — and then dispatches `_maybe_dispatch_substrate_entry_fill` / `_exit_fill` / `_stop_fill` for the engine-side effects. So a resting equity LIMIT or STOP_LIMIT that fills after `_wait_for_fill`'s 240s window **does** now reach the lifecycle row. The synchronous path still covers fills inside the window.
...
CREATE TABLE position_lifecycle_orders (
    ...
    applied_size_multiplier       REAL,                -- launch scaling, separate from caps
...
| `partial_close` | ✅ Wired | Operator Controls Phase C (PR #66, merged 2026-06-16). `reduce-position` writes a `partial_close` per-order row tagged `origin_kind='operator'` + `operator_command_uid` via `broker.close_position(partial_qty=...)`. The broker order may be `filled`, but its one matching `trades` row remains semantic `status='partial'`, carries realized P&L/R and timestamps, and refreshes the lifecycle P&L aggregate. The handler writes an absolute broker-derived residual, so handler-first and substrate-first quantity updates converge instead of subtracting twice. | Immediately on PR #66 merge. No bot-originated path creates partial closes; only the operator queue does. |
...
| `position_lifecycle_orders` (new) | Durable per-order intent (order_type, order_class, TIF, intended prices, qty). Decision-time sizing evidence (`risk_budget_dollars`, `approved_risk_dollars`, `risk_clip_kind`, `applied_size_multiplier`). Broker identity (`order_id`, `client_order_id`). Per-order lifecycle state and reconciliation anchor. Pre-fill slippage benchmark provenance. Relationships between orders (parent / replaces). Origin (bot vs. operator command). | Position-level rollups. Computed realized slippage. Trade-row primary keys. |
