...
Exits use **closing combo orders** through a bounded sequence of increasingly
marketable net-debit limit rungs. The strongest exit reasons retain a final
market fallback. Missing quote data never invents a price or blocks the
autonomous fallback. The 11.41 paper review observed every close resolving on a
limit rung and accepted the walk unchanged; reopen only on a concrete close
failure or market-fallback event.
