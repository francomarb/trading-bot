...
- Heartbeat-safe command polling runs independently of slow strategy cycles.
  ...
  `resolve-unexpected-protection` — remain `pending` until the engine thread
...
continues to claim handlers that do not read engine-owned stores, while
close/reduce/cancel and proof-gated unexpected-protection resolution stay
durably `pending` until the engine thread can claim and execute them during
...
Option trailing state is the latest §17.2 consumer adoption (PR #71, `docs/order_lifecycle_state_machine.md` §10.4): `option_trailing_stops.lifecycle_order_id` FKs into `position_lifecycle_orders(id)`, so broker identity / status for the trailing stop is read from substrate via a JOIN. The denormalized `alpaca_stop_order_id` / `stop_order_status` columns remain as a migration fallback and will be removed in a follow-up cleanup PR once any remaining out-of-tree readers have migrated to the FK join. Dashboard, health monitor, allocator, and reporting consumers of trailing-stop state should read substrate-authoritative identity through `OptionTrailingStopStore.get_by_occ_joined` (returns `JoinedOptionTrailingRow`) rather than the mirror columns when they next touch those code paths.
