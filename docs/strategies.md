...
The engine's ownership map (`_positions`) keys single-leg options by their durable lifecycle UID and carries the exact OCC symbol on the leg. Multi-leg positions also use UUID identity. Different contracts on one underlying can therefore remain independent.

**Cross-strategy conflict rule.** Single-leg and MLEG option positions may share an underlying. After contract selection, `_reject_if_contract_conflict` rejects any exact OCC already owned by another logical position. This is direction-agnostic because Alpaca aggregates or nets the same contract. The initial single-leg policy permits one active position per `(strategy, underlying)`.
...
**Same-underlying ownership:** single-leg options use the durable lifecycle UID as `position_id`; the exact OCC is their broker aggregation key. Different strategies may hold different contracts on SPY or QQQ independently, while exact-contract overlap is rejected.
