...
- **2026-09-02 calibration follow-up:**
  ...
  under the freshness plus 10 bps spread contract. Every shared
  execution-quality consumer excludes older unversioned arrival rows;
  existing combo-limit evidence is unchanged.
...
| 4 | Dashboard Recent Trades surfaces `slippage_benchmark_kind` + `slippage_measurement_quality` | + `load_trades` empty-frame columns; 2026-09-02 follow-up adds an operator-readable `Measurement Note` without mislabelling every `unavailable` row as a stale quote | ✅ |
