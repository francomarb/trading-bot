...
`sector/resolver.py` — `SectorResolver` maps stock tickers to sector labels using yfinance metadata cached in `data/cache/sector_map.json`. After startup broker reconciliation, ownership/lifecycle restoration, and protective-stop repair, `resolver.hydrate(all_symbols)` refreshes at most ten missing or 90-day-old classifications; no provider calls occur during the live trading loop. Never-attempted missing symbols lead initially, after which missing and stale rows share a least-recently-attempted queue so an unmappable symbol cannot starve valid stale rows. Cache rows include provider, schema, fetched-at, and last-attempted provenance. Each attempt is written through a same-directory temporary file and atomic replacement. A failed refresh preserves the last known classification, while manual overrides remain authoritative. The ten-attempt budget deliberately makes recovery from an empty cache or a large watchlist addition gradual across restarts. Industry takes priority over sector in normalization (NVDA → industry="Semiconductors" → `"semiconductors"`, not `"technology"`). ETFs return `None`. Unknown symbols fail open.
...
**11.48 (verified 2026-08-28) — "stop-risk first" is enforced.** Per-strategy risk targets (`STRATEGY_RISK_PER_TRADE_PCT`: donchian 0.40%, sma 0.60%, rsi 0.25% of account equity) size normal equity entries; concentration, gross, cash, and sleeve caps only reduce quantity. The pending order durably records the decision budget, approved risk, binding cap, and separately applied launch multiplier so late fills remain auditable. Fill-time risk is reported separately because price improvement or slippage is not a sizing decision. Derivation and maintenance triggers: [`allocator_risk_target_reconciliation.md`](allocator_risk_target_reconciliation.md).

The isolated options pool uses the same `SleeveAllocator` and HWM drawdown gate as equity strategies. In paper development that gate is observation-only by default (`PAPER_STRATEGY_DRAWDOWN_GATE_ENABLED=false`), while live retains its normal and catastrophic protection tiers. Single-leg option P&L is recorded with a 100× contract multiplier so the drawdown gate operates on real dollar amounts, not premium points. Credit-spread sleeve usage is based on defined max loss (`width − credit`) × contracts × 100, and realized spread P&L is fed back into the same allocator HWM gate on close.
...
| `_maybe_dispatch_substrate_stop_fill` | Resolves the exact OCC position, records the lifecycle UID, and logs with `log_stop_fill`. Replaced `_process_stream_stop_fills`, removed 2026-08-14 |
| `inspect_open_positions` exit | Calls `_OCC_PAT`-gated multiplier; skips `_record_fill`; uses premium as modeled price |

**WebSocket stream stop fills:** When a standalone option protective stop fills, the stream delivers the exact OCC symbol. It is translated to a substrate `OrderEvent`, applied by `apply_order_event`, and dispatched by `_maybe_dispatch_substrate_stop_fill`, which resolves the lifecycle UID, records P&L with the 100× multiplier, and calls `log_stop_fill`. **This is the only path.** Duplicate events are rejected by the substrate's compare-and-set rather than by a trade-log lookup.

**Position ownership model:** `_positions: dict[position_id, Position]` uses the ticker for equities and the durable lifecycle UID for single-leg options. Spreads also use a UUID. The exact OCC symbol carried by each leg is the broker aggregation and conflict boundary.

**Startup ownership authority:** The broker snapshot decides which positions physically exist. For each broker-held single-leg position, the matching non-terminal `position_lifecycle` row supplies the durable strategy owner and exact symbol. Historical `trades` replay is used only when no lifecycle claim exists, and fixed slot-order matching is the final legacy fallback. A lifecycle `error` row still owns its database key; unreadable lifecycle state, an unknown strategy, or an exact-contract mismatch therefore enters restricted conflict handling instead of silently falling through. This authority change is limited to ownership: realized P&L, fill economics, and other accounting remain trade-log sourced. MLEG spread restoration remains trade-ledger based until spread lifecycle rows hold the complete legs and entry economics needed to rebuild the runtime spread object.
...
Together: single-leg and MLEG positions may share an underlying, and two single-leg strategies may hold different contracts on it. Any exact OCC overlap is rejected. Two rolling 24h counters (`symbol_conflicts_24h`, `contract_conflicts_24h`) ride into `engine_state.json` for the HealthAssessor L1 check.
...
| `closing=True`, canceled | Keep the position open; finalize the substrate close row at the observed terminal status so the `uniq_one_active_close_per_position` guard releases and the exit path retries next cycle. The current implementation does not escalate automatically to a more marketable debit or to a market order after timeout. |

**Pre-registration.** When a worker is dispatched, the engine immediately creates a two-leg `Position` (UUID `position_id`, `position_type='spread'`, both legs) and calls `strategy.register_spread(OpenSpread(...))`. The drain confirms or rolls back. A pending close row in `position_lifecycle_orders` (§10.7) is what guards against double-submitting a close while one is in flight — durable across restarts.

**Trade-DB layout for spreads.** `log_spread_fill` writes **one row per leg** (both keyed by the same `position_id`, `position_type='spread'`). The net economics ride the short-leg row; the long-leg row carries NULL for `avg_fill_price` and `entry_reference_price` because Alpaca's combo result does not expose individual leg prices. Realized P&L on a close rides the short-leg close row so `read_strategy_realized_pnl_summary` (which now counts `position_type='spread'` rows alongside single-leg sells) folds spread P&L into the HWM gate after a restart. Spread rows are deliberately **excluded** from `read_all_open_owners` / `read_owner_for_symbol` — a spread leg is not a standalone single-leg position; the long-leg's `side='buy'` row would otherwise be mistaken for a single-leg open.
...
- This is the explicit exception to lifecycle-first startup ownership. Current spread lifecycle rows identify the parent and legs but do not preserve all entry reconstruction inputs, so changing spread authority now would discard information rather than improve it.

**Entry-guard bypass.** The standard "already hold this symbol, skip re-entry" guard is skipped for spread strategies; their own per-instance caps govern concurrency. Both MLEG and single-leg option strategies skip the underlying-level equity guard and instead run `_reject_if_contract_conflict` after resolving exact OCC legs.
...
Every row carries a `position_id` and `position_type`. Equity rows use the ticker; lifecycle-backed single-leg option rows use `position_uid` (legacy rows without one retain the exact OCC); spread rows use their parent UUID. Legacy single-leg owner queries exclude spreads by construction.
...
20. Keep one active single-leg position per `(strategy, underlying)` initially, and rely on the exact-OCC conflict guard to prevent broker aggregation across strategies
...
- Never bypass the exact-OCC conflict guard; Alpaca aggregates the same contract even when strategies are logically separate
