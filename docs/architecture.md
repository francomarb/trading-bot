...
**Cross-strategy conflict rule (PLAN.md 11.44):** the conflict guard is split by asset class. **Equity**-vs-equity entries on the same ticker remain blocked at the top of `_process_symbol` via `_get_owner` — the broker aggregates equity positions by symbol so shared ownership cannot be represented and the alert code is `SYMBOL_CONFLICT`. **Options** strategies (any strategy exposing `build_option_execution` or `build_spread_execution`) are exempt from the underlying-level check — distinct OCC contracts on the same underlying (different strike / expiry / right) are separate broker positions and must be allowed to coexist. The real safety concern — two strategies dispatching against the *exact same OCC contract*, which the broker would aggregate into one position with ambiguous ownership — is enforced at dispatch time by `_reject_if_contract_conflict`. It runs leg-level checks against every tracked position via `_contract_owner` immediately after the option picker resolves the OCC (single-leg path) or `build_spread_execution` returns the plan (MLEG path), and fires the distinct `CONTRACT_CONFLICT` alert code on collision. The two rolling 24h counters (`symbol_conflicts_24h`, `contract_conflicts_24h`) ride into `engine_state.json` for the HealthAssessor L1 check. The rule is direction-agnostic on purpose: long-vs-short on the same OCC nets at the broker and would silently corrupt ownership tracking.
