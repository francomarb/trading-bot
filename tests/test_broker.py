...
class TestLifecycleOrdersSubstrate:
    """Verify equity / fractional / options entry submission writes per-order
    substrate rows at status='pending' and attaches the broker-assigned
    order_id on submit return.

    These are the only contracts commit 6 adds — apply_order_event terminal
    transitions are wired in subsequent commits (stream / cycle / startup
    reconciliation). The substrate insertion is best-effort; the helpers
    swallow exceptions, so test that the helpers ARE called with the right
    arguments rather than asserting on row state directly.
    """

    @staticmethod
    def _broker_with_substrate(api: MagicMock, lifecycle_orders_store: MagicMock):
        broker = AlpacaBroker(
            client=api,
            max_attempts=1,
            base_delay=0.0,
            lifecycle_orders_store=lifecycle_orders_store,
            lifecycle_store=MagicMock(),
        )
        # PositionLifecycleStore.create_pending returns the uid; mimic it
        # so _lifecycle_begin returns a real string.
        broker._lifecycle_store.create_pending = MagicMock(return_value="pos-uid-test-1")
        return broker

    def test_equity_market_entry_inserts_pending_then_attaches_order_id(self):
        api = MagicMock()
        api.submit_order.return_value = _alpaca_order(id="alpaca-ord-1", status="filled")
        api.get_order_by_id.return_value = _alpaca_order(id="alpaca-ord-1", status="filled")
        orders_store = MagicMock()
        broker = self._broker_with_substrate(api, orders_store)

        broker.place_order(_decision(stop=95.5), poll_timeout=0.1)

        # insert_pending called with role='entry_primary', order_class='oto',
        # client_order_id matching what we built.
        orders_store.insert_pending.assert_called_once()
        kwargs = orders_store.insert_pending.call_args.kwargs
        assert kwargs["role"] == "entry_primary"
        assert kwargs["order_class"] == "oto"
        assert kwargs["side"] == "buy"
        assert kwargs["order_type"] == "market"
        assert kwargs["intended_qty"] == 10.0
        assert kwargs["intended_stop_price"] == 95.5
        assert kwargs["entry_reference_price"] == pytest.approx(100.0)
        # position_uid is generated by engine.lifecycle.new_position_uid;
        # match the prefix rather than the exact value (create_pending is
        # called *with* this uid, not as a return path).
        assert isinstance(kwargs["position_uid"], str)
        assert kwargs["position_uid"].startswith("pos_")
        assert kwargs["client_order_id"].startswith("sma_crossover-")

        # attach called once with the same client_order_id + broker's order.id.
        orders_store.attach_broker_order_id.assert_called_once()
        attach_kwargs = orders_store.attach_broker_order_id.call_args.kwargs
        assert attach_kwargs["client_order_id"] == kwargs["client_order_id"]
        assert attach_kwargs["order_id"] == "alpaca-ord-1"

    def test_equity_limit_entry_records_intended_limit_price(self):
        api = MagicMock()
        api.submit_order.return_value = _alpaca_order(status="accepted")
        api.get_order_by_id.return_value = _alpaca_order(status="accepted", filled_qty=0)
        orders_store = MagicMock()
        ...
        # Broker rounds the price to 2dp before submission; that rounded
    ...
    def test_substrate_insert_skipped_before_dry_run(self):
        ...
        broker.place_order(_decision(), poll_timeout=0.0, poll_interval=0.0)
        orders_store.insert_pending.assert_not_called()
        orders_store.attach_broker_order_id.assert_not_called()

    def test_substrate_insert_skipped_when_store_not_wired(self):
        """When no orders store is injected, behavior is byte-for-byte
        identical to pre-foundation broker — no substrate calls at all."""
        api = MagicMock()
        api.submit_order.return_value = _alpaca_order(status="filled")
        api.get_order_by_id.return_value = _alpaca_order(status="filled")
        broker = AlpacaBroker(client=api, max_attempts=1, base_delay=0.0)
        # No lifecycle_orders_store and no lifecycle_store: the broker is
        # entirely back to its pre-commit-6 surface.
        result = broker.place_order(_decision(stop=95.5), poll_timeout=0.1)
    ...
    def test_attach_failure_does_not_block_order(self):
        ...
        api.get_order_by_id.return_value = _alpaca_order(status="filled")
        orders_store = MagicMock()
        orders_store.attach_broker_order_id.side_effect = RuntimeError("conflict")
        broker = self._broker_with_substrate(api, orders_store)
