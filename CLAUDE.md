...
- Data API (market data) — feed strategy:
  ...
  - **Live engine, paper account**: `feed="iex"` (free real-time stock bars and arrival quotes on basic tier). This is the default main stock-feed switch in `ALPACA_DATA_FEED`; do not select `sip` until real-time SIP entitlement is active and verified.
