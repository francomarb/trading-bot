...
| `11.47` Donchian stop-limit entries | 2026-06-11 audit of Donchian sleeve (1/18 winners, -$3.6k cumulative) showed two structural failure modes from the market-on-open entry shape: gap-up chase (QCOM #1 5/11 — driver behind PLAN `11.32`) and failed-breakout entry on gap-down (QCOM #2, ARM, MRVL, ASML 6/01-6/05 — fills $400-$900 below signal because price collapsed before fill but bot still bought). Convert Donchian entries from market-on-open to broker-resting stop-limit at the breakout level with chase cap baked into the limit price. Hybrid sizing: whole-share leg = stop-limit (structural protection); fractional residual (only when sizing produces a fraction) = existing market path with 11.32 guard + downside gate (skip residual if live quote < stop). EOD cancellation for unfilled stop-limits (DAY TIF + defensive sweep). Orphan residual cleanup at EOD if residual filled but stop never triggered. Supersedes the earlier proposed `11.36` live-quote pre-screen — stop-limit's trigger condition absorbs that check structurally. `11.32` remains as universal pre-submit guard for all market orders (other strategies + Donchian residual). | Paper-mode observation — no fixed window. Watch the first handful of Donchian fills for: (a) any STOP_LIMIT filled above its limit (would be a broker contract violation — must be zero); (b) any fill on the whole-share leg below the trigger (would indicate failed-breakout slip — must be zero); (c) fractional residual gate behavior (live < trigger skips submission, log captures the decision); (d) any orphan-residual close from the EOD sweep (log line confirms the sweep fired). Promote follow-up tightening only on observed evidence; this is paper, not a calibration gauntlet. |
| `11.48` Donchian forward-looking breakout entries | PR #58 R6 review flagged that 11.47's stop-limit fires at bar t+1 against a trigger derived from the prior-N-day high that was ALREADY crossed at bar t close. This means 11.47 protects against gap-down (failed breakout) and gap-up (chase cap) but does not capture the "moment of breakout" — by bar t+1 the breakout is old news. A stricter Turtle-style implementation would place a stop-limit at bar t-1 close, looking forward to bar t with the rolling-max-through-t-1 as the trigger. The broker then fires the moment price first crosses that level intraday on bar t. **Why deferred from 11.47**: this is a fundamental shift in signal semantics (forward-looking placement, no close confirmation), not a bug fix. 11.47 ships with the chase + failed-breakout protection the original 2026-06-11 audit was scoped against; 11.48 is a separate strategy redesign that needs its own backtest, sizing review, and paper observation before promotion. | Design doc draft covering: (a) trigger-placement timing (bar t-1 close, not bar t close); (b) interaction with the regime gate and edge filter (both currently evaluate at bar t close — would need to re-evaluate intraday on fill?); (c) the "stale level" risk if the broker's resting order survives across multiple bars; (d) backtest semantics that distinguish "signal fired before the close" vs "signal confirmed at close." Decide between this and the current 11.47 model based on the design tradeoffs and any 11.47 paper-fill outliers that this would address. |
| `11.49` Backtest hybrid residual modeling | PR #58 R6 review flagged that `backtest/runner.py::_apply_stop_limit_semantics` models a single stop-limit fill per entry signal. The hybrid path in production fires a separate fractional MARKET residual when the live quote is at-or-above the trigger; the backtest does not simulate this second leg. Effect on backtest: position size for hybrid entries is understated by the fractional residual qty (≤0.99 shares, ~15% on the highest-priced names in the Donchian universe). The fill-price model and chase/failed-breakout protection are accurate as far as they go; only sizing fidelity is short. **Why deferred from 11.47**: vectorbt sizes positions internally via cash allocation, so injecting per-bar residual qty requires extending the fill-price helper to also yield a size vector and reworking the `Portfolio.from_signals` call. Not blocking the 11.47 audit hypothesis but matters for any future Donchian backtest-vs-paper comparison. | Extend `_apply_stop_limit_semantics` to emit (entries, fill_price, size_multiplier) tuples. When the open is at-or-above the trigger, include the residual fraction; when gated out, use only the whole-share size. Document in `docs/donchian_breakout_strategy.md` what the backtest does and does not model. |
...
- Strategies that trigger on a price-level cross (Donchian) submit broker-side stop-limit orders rather than market-on-open. Decision cadence is still "completed bars only" — yesterday's closed bars define the level; the broker handles the intraday trigger. The limit price encodes the chase cap structurally. Per `11.47`.
