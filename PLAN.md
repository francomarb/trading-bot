...
| 9.2 | Trade log CSV: `timestamp, symbol, side, qty, price, order_id, strategy, reason, stop_price, modeled_slippage_bps, realized_slippage_bps` | ✅ |
...
| 9.8 | Verified: logs + trade CSV + daily summary + per-strategy attribution written correctly after a paper trade cycle | ✅ (2026-04-16) |

**Exit Criteria:** Every trade is in the CSV with slippage data. Every day produces a P&L summary with per-strategy breakdown. All operator-critical events alert.
...
| 9.5.2 | `backtest/reconcile.py` — script that, given a date range, compares: (a) paper fills from trade CSV vs. (b) backtest-predicted fills on the same bars | ✅ |
...
| 10.7 | Separate live log file / trade CSV (never co-mingled with paper history) | ⬜ |
| 10.8 | Verified: bot connects to live Alpaca endpoint in dry-run, then with 1-share manual-approval order | ⬜ |

**Exit Criteria:** Bot can switch to live via config. First live order requires explicit human approval. Hard dollar cap is enforced. Paper and live data are never co-mingled.
