...
| Operator controls Phase A + B (`docs/operator_controls_proposal.md`) | Live trading needs precise, audited operator intervention rather than only the blunt stop/edit/restart options available today | Phase A (PR-1 + PR-2): `position_uid` lifecycle identity + `operator_commands` queue + sticky halt CLI ✅ shipped + baked ≥10 days. Phase B: 4 soft-control actions (pause-entries / resume-entries / pause-strategy / resume-strategy), fast operator-command heartbeat thread (5s drain), Telegram `/halt` migrated to queue (single channel), `position_uid` threaded through fill/exit alerts. Ready to bake on paper before destructive Phase C. |
