...
| 5 | `11.72` shared watchlist fundamentals parsing | **Implementation and narrow corrections complete on PR branch.** ISRG replaces LIN in Donchian; BRK.B replaces the RSI ranked slot displaced on the frozen promotion window, while the broader one-scan RSI liquidity churn was rejected. Lifecycle protection remains intact | Merge PR #161, pull main, and recycle only with `./recycle_bot.sh`; verify startup ownership and the 100/50 ranked pool counts before closing the item |
...
| 7 | RSI watchlist refresh | ✅ Paper exercised before the `11.72` correction; the PR now preserves 50 ranked names plus three lifecycle-protection members | After merge/recycle, verify all protected positions retain ownership and stops. Remove each protection member only after it is flat and terminal |
...
| `11.72` Shared watchlist profitability parsing and solvency provenance | **IMPLEMENTED WITH OPERATOR-APPROVED NARROW CORRECTIONS ON PR BRANCH.** The shared parser preserves exact `Net Income` precedence, adds the narrow `Net Income Common Stockholders` fallback and source provenance, and exposes explicit solvency reasons. RSI, Donchian, SMA, the standalone report, and the historical RSI builder now fail closed consistently. ISRG replaces LIN in the Donchian ranked 100. BRK.B replaces BAC in the RSI ranked 50 on the frozen promotion window; required lifecycle protection remains appended outside the ranked pool. The extra COHR/JNJ versus COST/GLW/TXN current-snapshot turnover was rejected as ordinary liquidity drift. Private ledger and broker checks found no orphaned removed position or unresolved order. | Merge PR #161, pull main, recycle only with `./recycle_bot.sh`, and verify startup ownership, stops, and the 100/50 ranked opportunity counts. Then close `11.72`; continue quarterly/stability-triggered refreshes rather than one-scan churn. |
