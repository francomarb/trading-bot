...
| 10.F9 | **Sector Momentum Gauge** — `sector/resolver.py` + `sector/gauge.py`. Resolver maps ticker → sector via yfinance (industry-first normalization, JSON cache at `data/cache/sector_map.json`, hydrated at startup with retry/timeout). Gauge scores 12 sector ETFs as HOT/NEUTRAL/COLD (5-signal composite: SMA200, SMA50, golden cross, SMA50 distance, volume confirmation). `SectorMomentumFilter` adapter with configurable `cold_policy` ("block"/"warn"/"pass"). `CompositeEdgeFilter` AND-chains any list of edge filters. Per-strategy policies: RSI `cold_policy="block"` (cluster risk), SMA/Donchian `cold_policy="warn"` (trend strategies can catch sector turns). All exits unaffected. 66 new tests (38 resolver + 28 gauge/filter/composite). | Medium | 🔴 | ✅ (2026-05-01) |
