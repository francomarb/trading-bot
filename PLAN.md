...
| 11.22 | **Phase 2 filter-decision migration** — complete the rollout of the new structured edge-filter decision contract (`allowed + reasons`) across active first-party filters after the mixed-mode Phase 1 observation window is deemed healthy. Phase 1 is complete: `BaseStrategy` now normalizes both legacy `pd.Series` filters and structured decision filters; the **Bollinger Breakout** filter implementation (`BollingerSqueezeEdgeFilter`) is the first real migrated filter; engine snapshot and dashboard reason rendering work in mixed mode. The repo standard is now the structured contract; plain boolean `pd.Series` filters remain compatibility-only during the transition and should be treated as the old method being phased out. **New first-party strategies/filters must use native `EdgeFilterDecision` output and must not introduce new mutable `get_last_block_reasons()` side-channel state.** Phase 2 scope: migrate `SMAEdgeFilter`, `RSIEdgeFilter`, `DonchianEdgeFilter`, `SectorMomentumFilter`, and `SPYOptionsEdgeFilter` off mutable `get_last_block_reasons()` side state; keep legacy `pd.Series` support only as compatibility scaffolding for trivial or external filters. **Completed so far:** Bollinger Breakout (`BollingerSqueezeEdgeFilter`), `RSIEdgeFilter`, and `SPYOptionsEdgeFilter`. **Remaining:** `SMAEdgeFilter`, `DonchianEdgeFilter`, and `SectorMomentumFilter`. Acceptance: active paper bot behavior unchanged, watchlist `Reason` column remains accurate, and the mutable side-channel can be removed from first-party filters. | 🔄 (Phase 1 complete; Phase 2 in progress — Bollinger Breakout, RSI, and SPY Options migrated) |
