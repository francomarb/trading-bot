...
| 5 | `11.72` shared watchlist fundamentals parsing | **Implemented on PR branch; universe decision pending.** Shared source provenance, `UNKNOWN` review verdicts, and consistent fail-closed reasons now cover RSI, Donchian, SMA, and the legacy builder. Candidate reports put ISRG at Donchian rank 84 and propose three RSI swaps | Review [`docs/watchlist_fundamentals_audit.md`](docs/watchlist_fundamentals_audit.md) and the candidate-report diffs; explicitly approve or reject each universe change before configuration updates or Donchian recycle |
...
| `11.72` Shared watchlist profitability parsing and solvency provenance | **IMPLEMENTED ON PR BRANCH — candidate-universe decision pending.** The shared parser preserves exact `Net Income` precedence, adds the narrow `Net Income Common Stockholders` fallback and source provenance, and exposes explicit solvency reasons. The standalone report now returns `UNKNOWN` for unavailable required facts; RSI, Donchian, and SMA distinguish provider errors, unknown facts, and genuine failures; SMA no longer gates on display text; the historical RSI builder requires affirmative solvency. Focused tests are 175 passed and the full suite is 3,855 passed. Review-only reports propose ISRG for LIN in Donchian and BRK.B/COHR/JNJ for COST/GLW/TXN in RSI. Current configuration still covers every open position. | Review [`docs/watchlist_fundamentals_audit.md`](docs/watchlist_fundamentals_audit.md) and the two `11.72` candidate reports. Approve, modify, or reject the RSI and Donchian membership diffs separately. If approved, append the privately identified lifecycle-protection holdings, update configuration and active-pool documentation, rerun tests, and recycle only with `./recycle_bot.sh`. Do not close `11.72` or recycle the PR #160 Donchian cohort before that decision. |
