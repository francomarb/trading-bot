...
| Strategy graduation evidence package | 🔄 Daily evidence deployed; first report validated | Lifetime and labeled date/strategy views expose lifecycle outcomes, forward marks, costs, and excluded trade-only history. Collect the first versioned cohorts before defining sufficiency rules. |
...
| 2 | ~~`11.70` SMA crossover entry-quality audit~~ | ✅ Closed — fixed SIP development/held-out comparison supports retaining the current entry and stop behavior | Reopen only if new paper evidence contradicts the expected clustered-loss / rare-runner payoff shape, or a materially different pre-registered entry proposal is made |
| 3 | `11.61a` entry-candidate observation | Deployed and collecting; ranking remains off | Collect real same-strategy contention, then build strategy-aware counterfactual resolution before proposing any ranking rule |
| 4 | `11.62` portfolio heat ceiling | Deferred while `11.60` has only one would-block event | Collect more sleeve-heat evidence before designing a portfolio-wide ceiling |
| 5 | RSI watchlist refresh | Durable-gate v3 pool promoted: 50 ranked names plus protected open ABNB/CCK; sector concentration accepted and GOOG-over-GOOGL enforced | Verify the recycled paper bot reports the promoted RSI slot and preserves ownership/stops for ABNB/CCK |
...
| ~~`11.70` SMA crossover entry-quality audit~~ | ✅ **CLOSED — retain current behavior.** The fixed SIP development/held-out comparison found that the structure-aware stop reduced drawdown but also expectancy, while the pullback avoided losers at the cost of blocking more winners and failed out of sample. The current clustered-loss / rare-runner payoff shape is accepted; no bot change is authorized. See [`docs/sma_entry_quality_audit.md`](docs/sma_entry_quality_audit.md). | Reopen only if new paper evidence contradicts the expected payoff shape, or a materially different entry proposal is pre-registered. Historical earnings/universe fidelity would be required before approving a future positive change, but is not a reason to keep tuning variants that already failed. |
...
| `11.61` Entry-candidate ranking | 🔄 **`11.61a` DEPLOYED AND COLLECTING; ranking off.** Permanent decisions record strategy-specific signal/filter facts, allocation/risk/execution context, selection order, and lifecycle links. A disposable shadow table is queued only for real same-strategy capacity contention. | Collect contention. Resolve refused candidates with strategy/instrument-correct fill and exit semantics before a separately reviewed `11.61b` ranking rule. Never compare scores across strategies. See [`docs/entry_candidate_observation.md`](docs/entry_candidate_observation.md). |
