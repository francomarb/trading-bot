...
| 4 | `11.71` runtime cache and cycle-latency audit | **AUDIT COMPLETE; SECTOR FRESHNESS SLICE IMPLEMENTED ON PR #162.** Sector classifications now refresh after 90 days under a ten-attempt startup budget, carry provider/schema/fetch provenance, preserve stale values on provider failure, and persist each attempt atomically. The unsafe pre-engine placement and unenforced time deadline remain open, alongside the approximately 11-minute nested bar-retry upper bound and market-data request burst. See `docs/runtime_cache_cycle_audit.md` | Finish the narrow implementation: move/hard-bound sector hydration after broker safety work; remove nested/final-sleep bar retries; quarantine corrupt Parquet; add phase, stock-transport request, retry, order-wait, start-lag, and position-latency telemetry. Let events authorize batching or other follow-ups |
...
| `11.71` Runtime cache, startup hydration, and cycle-latency audit | **PHASE A AUDIT COMPLETE; SECTOR FRESHNESS/DURABILITY SLICE IMPLEMENTED ON PR #162.** The resolver now refreshes 90-day-old classifications oldest-first under a ten-attempt startup budget, records provider/schema/fetch provenance, treats legacy rows as stale, preserves last-known data on provider failure, and atomically saves after every attempt. Unsafe pre-engine hydration and the unenforced time deadline remain open. The audit also confirmed an approximately 11-minute single-symbol nested-retry upper bound, unguarded Parquet reads, and a healthy-cycle burst near the Basic plan's 200 market-data calls/minute. See `docs/runtime_cache_cycle_audit.md`. | Finish safety-first, time-bounded sector hydration; one bounded bar-read retry policy with no final sleep; corrupt-Parquet quarantine; and phase/stock-transport-request/retry/order-wait/start-lag/position telemetry. Use event gates—not a five-session pause—to authorize native batching, completed-session refresh, position ordering, cadence changes, locks, or asynchronous settlement. Keep Donchian ranked size at 100. |
