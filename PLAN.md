...
| 2 | `11.70` SMA crossover entry-quality audit | Open investigation; current paper behavior remains unchanged | Run a production-faithful, fixed-risk, out-of-sample comparison before proposing any entry or stop change |
...
| `11.70` SMA crossover entry-quality audit | 🔎 **OPEN — investigation only; no bot change authorized.** Four recent closed SMA lifecycles all stopped near −1R, and several fills placed the 2×ATR stop near SMA50. Preliminary replay confirms that geometry but does not show that a fixed extension cap or naive pullback improves results; extended entries also contain major winners. | Build a production-faithful replay with actual filter, next-open/fill-anchored-stop, fixed-risk, and non-overlap semantics. Compare the current control with a structure-aware stop and a precisely defined pullback entry across development and held-out periods. Report expectancy, drawdown, stop rate, winners blocked, and losers avoided. Change paper behavior only if improvement is stable out of sample. |
