...
| Realized-P&L row repair (2026-07 audit) | 13 pre-fix single-leg exit rows booked realized P&L against a reference-price or position-blended entry basis (five donchian stops incl. trade 320, one sma stop, fractional-residual closes, two NULL option stop fills). Donchian's DB-restored running P&L (−3,263 vs fill-based truth −2,578) already breaches its 15% sleeve-drawdown threshold on phantom losses — only the trade-count floor (19 < 25) keeps entries enabled. Writers are fixed (chronological replay + broker-fill-only basis with lifecycle-substrate fallback); the historical rows still need the one-time repair. | Operator: stop the bot, dry-run `venv/bin/python scripts/repair_stop_fill_pnl.py`, review (expected per-strategy deltas: donchian +685.07, sma +385.33, spy_options −351.00), run with `--apply`, restart. Allocator P&L/HWM rehydrates from the repaired rows on startup — no engine_state.json edit. Delete row once applied. |
