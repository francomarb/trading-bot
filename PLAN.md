...
| 5 | `11.72` shared watchlist fundamentals parsing | Yahoo net-income row variants can make a profitable company look indeterminate; ISRG is the verified current example. Failing closed is safe, but the RSI and Donchian reports cannot distinguish unavailable profitability data from a genuine solvency failure | After PR #160, fix and test the shared parser in a focused PR, split unknown from failed solvency in both scanners, and review regenerated universe diffs before any configuration change or Donchian recycle |
...
| `11.72` Shared watchlist profitability parsing and solvency provenance | **OPEN — verified review gap.** `scripts/watchlist_review.py` currently recognizes only Yahoo's exact `Net Income` row. Some profitable companies expose equivalent annual rows under different labels; ISRG therefore produces `is_profitable=None` and is conservatively rejected. The shared RSI and Donchian scanners then group indeterminate profitability with genuine failed-solvency/runway results. This is a false-exclusion and auditability problem, not an unsafe inclusion. | Implement in a separate focused PR after #160: define and test an ordered set of semantically valid annual net-income aliases; retain `None` when none is available; distinguish `solvency_unknown` from an affirmative solvency/runway failure in both scanner reports and explanations; regression-test positive, negative, fallback, and missing-data cases. Regenerate both RSI and Donchian reports, compare membership and boundary changes, preserve open-position lifecycle coverage, and require explicit operator review before changing either configured universe. Complete before the next routine watchlist refresh and before recycling the Donchian cohort introduced by #160. |
