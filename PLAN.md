...
| 4 | `11.71` runtime cache and cycle-latency audit | **AUDIT COMPLETE; SECTOR FRESHNESS/SAFETY SLICE IMPLEMENTED ON PR #162.** Sector classifications refresh after 90 days under a fair ten-attempt startup budget, carry provenance, preserve stale values on failure, and persist atomically. Classification refresh and the sector-ETF heat snapshot now run only after broker ownership/lifecycle reconciliation and protective-stop repair. The resolver's configured elapsed deadline remains unenforced, alongside the approximately 11-minute nested bar-retry upper bound and market-data request burst. See `docs/runtime_cache_cycle_audit.md` | Finish the narrow implementation: enforce sector elapsed deadlines; remove nested/final-sleep bar retries; quarantine corrupt Parquet; add phase, stock-transport request, retry, order-wait, start-lag, and position-latency telemetry. Let events authorize batching or other follow-ups |
| 5 | `11.72` shared watchlist fundamentals parsing | ✅ **COMPLETE.** PR #161 merged, main was pulled, and the bot was recycled via `./recycle_bot.sh`. Startup ownership/stops and the 100 Donchian / 50 RSI ranked opportunity counts were verified; lifecycle-protection members remain appended outside those ranked pools. [Sanitized operational evidence](https://github.com/francomarb/trading-bot/pull/161#issuecomment-5753960144) | Continue quarterly/stability-triggered refreshes rather than reacting to one-scan liquidity churn |
...
| 7 | RSI watchlist refresh | ✅ Paper exercised and post-recycle ownership/stops verified; the deployed list preserves 50 ranked names plus three lifecycle-protection members | Remove each protection member only after it is flat and terminal |
...
| RSI 50-candidate static-pool refresh | ✅ **PROMOTED 2026-09-09; NARROWLY CORRECTED ON PR BRANCH 2026-09-20.** Membership uses only durable price, dollar-liquidity, size, and affirmative-solvency gates. BRK.B replaces the frozen-window boundary name displaced after provider normalization; broader one-scan liquidity churn was rejected. The configured list retains three lifecycle-protection members after the ranked 50. | New config-hash evidence cohort begins on recycle. Remove each lifecycle-protection member only after it is flat and terminal; otherwise continue ordinary paper evidence collection. |
...
| `11.72` Shared watchlist profitability parsing and solvency provenance | ✅ **COMPLETE.** The shared parser preserves exact `Net Income` precedence, adds the narrow `Net Income Common Stockholders` fallback and source provenance, and exposes explicit solvency reasons. RSI, Donchian, SMA, the standalone report, and the historical RSI builder now fail closed consistently. ISRG replaced LIN in the Donchian ranked 100; BRK.B replaced BAC in the RSI ranked 50 on the frozen promotion window; required lifecycle protection remains appended outside the ranked pool. PR #161 merged, main was pulled, and the post-recycle ownership, stops, and ranked counts were verified. [Sanitized operational evidence](https://github.com/francomarb/trading-bot/pull/161#issuecomment-5753960144) | Continue quarterly/stability-triggered refreshes rather than one-scan churn. |
| ~~`11.73` Donchian parameter rebaseline on the durable pool~~ | ✅ **COMPLETE — retain 30/15.** The review-corrected shared-sleeve SIP comparison includes production's pre-sizing $100 remaining-capacity floor. 55/20 passed the return-frequency, Sharpe (1.02 vs 0.78), drawdown, and mean-R gates, but failed both concentration sensitivities: excluding 2024 left 10.0% vs 10.5%, and excluding top contributor PLTR left 12.1% vs 14.1%. The expanding selector chose 30/15 three times and 20/10 twice, never 55/20. This is a fragile current-cohort robustness result, not a survivorship-free selector test or authority over paper outcomes. See [`docs/donchian_parameter_rebaseline.md`](docs/donchian_parameter_rebaseline.md). | Keep 30/15 and collect the new universe's paper cohort. Reopen parameters only with materially new forward evidence and a new pre-registration; do not salvage 55/20 by changing this test after the result. |
| `11.74` Donchian deployment validation | 🔄 **IN PROGRESS — plausible core signal, unproven deployment, strongly adverse initial regime.** Production-mirror/reconciliation identity and exit-observation defects are repaired; the corrected 100-name pool and configuration are now frozen. `11.73` supplied a shared-capital STOP_LIMIT/concurrency study and a fixed 2021–2025 held-out comparison for close-based variants on the expanded pool, but deliberately left the 4R heat cap unenforced and did not compare classic high/low Turtle semantics or the original 32-name universe. See [`docs/donchian_deployment_validation.md`](docs/donchian_deployment_validation.md). | Complete the pre-registered missing comparison without changing paper settings. Keep Donchian paper-only until at least 25 trusted exits under the frozen configuration **and** enough independent entry clusters exist; symbol count alone is not evidence count. |
| `11.71` Runtime cache, startup hydration, and cycle-latency audit | **PHASE A AUDIT COMPLETE; SECTOR FRESHNESS/DURABILITY/SAFETY SLICE IMPLEMENTED ON PR #162.** The resolver refreshes 90-day-old classifications under a fair ten-attempt startup budget, records provenance, treats legacy rows as stale, preserves last-known data on failure, and atomically saves every attempt. Classification and sector-ETF heat refresh now run only after ownership/lifecycle reconciliation and protective-stop repair; hook failure cannot block the first cycle. The configured sector elapsed deadline remains unenforced. The audit also confirmed an approximately 11-minute single-symbol nested bar-retry upper bound, unguarded Parquet reads, and a healthy-cycle burst near the Basic plan's 200 market-data calls/minute. See `docs/runtime_cache_cycle_audit.md`. | Finish enforced sector elapsed deadlines; one bounded bar-read retry policy with no final sleep; corrupt-Parquet quarantine; and phase/stock-transport-request/retry/order-wait/start-lag/position telemetry. Use event gates—not a five-session pause—to authorize native batching, completed-session refresh, position ordering, cadence changes, locks, or asynchronous settlement. Keep Donchian ranked size at 100. |
