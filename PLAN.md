...
| 11.25 | **SPY options contract-picker refinements** — the 2026-05-08 hotfix made contract selection safe via strike-band constraint + pagination. This change improves *quality*: candidates are now ranked by a composite score across strike proximity (0.45), spread quality (0.35), and premium efficiency (0.20), with hard filters dropping unaffordable, broken-spread (>10%), invalid-quote, and premium-outlier candidates before scoring. Selection caps quote fetches at the top-5 strike-nearest candidates. The strategy passes the allocator's `notional_cap` straight through to the ranker as `max_premium_per_contract`; the ranker computes per-contract cost as `mid × 100` and compares it to that dollar budget, so the picker never returns a contract the sleeve cannot afford. No `/100` conversion — both sides are in the same dollars-per-contract units. Selection logs the top contract's score breakdown plus up to 3 runners-up for explainability. New module `utils/options_ranker.py` (pure logic); `utils/options_lookup.find_best_call` now requires `quote_lookup` callback and `max_premium_per_contract`; `SPYOptionsReversionStrategy.build_option_execution` accepts `notional_cap` and builds an OPRA snapshot-backed quote lookup. | ✅ (2026-05-10 — branch `feat/11.25-options-contract-picker-quality`; 21 new tests, 973 total passing) |
