...
| `11.73` Donchian parameter rebaseline on the durable pool | 🔄 **PRE-REGISTERED.** Compare standard 20/10, 30/10, current 30/15, and 55/20 channels with shared sleeve capital, production gates, STOP_LIMIT mechanics, static stops, and five annual held-out folds. The current 100-name snapshot makes this a fixed-cohort temporal robustness study, not a survivorship-free selector backtest. See [`docs/donchian_parameter_rebaseline.md`](docs/donchian_parameter_rebaseline.md). | Implement and test the frozen harness, warm the deep SIP cache, run without changing the criteria, and publish the result. No configuration change is self-authorized. |
