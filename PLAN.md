...
| RSI3 simplification paper-watch (`11.23`) | 🔄 9 entries / 6 completed exits / 3 open | The six completed outcomes are split 3 wins / 3 losses and total about -$101. The sample remains too small to judge expectancy or clustered dip-buying. |
...
| Strategy graduation evidence package | 🔄 Versioned cohorts collecting | Reporting is validated. Only SPY options has a completed versioned lifecycle so far; the other current cohorts are open or unfilled. Keep collecting before defining sufficiency rules. |

---

## Active Work Queue

### Current Priority View — 2026-09-12

| Priority | Item | Current State | Next Action |
|---:|---|---|---|
| 1 | Trustworthy strategy graduation report | Deployed and validated; first versioned cohorts are collecting, with one completed SPY-options lifecycle | Keep collecting current-version outcomes, then define evidence-sufficiency rules. Preflight binding remains a later PR |
| 2 | ~~`11.70` SMA crossover entry-quality audit~~ | ✅ Closed — fixed SIP development/held-out comparison supports retaining the current entry and stop behavior | Reopen only after ≥30 additional current-configuration closes contradict the expected payoff shape, or with a materially different pre-registered proposal plus an earnings/history fidelity plan |
| 3 | `11.61a` entry-candidate observation | First real contention captured; RSI shadow resolution is available offline and ranking remains off | Refresh the two open refused-candidate shadows until they exit; require repeated resolved contention before proposing a ranking rule |
| 4 | `11.71` runtime cache and cycle-latency audit | Newly opened after the Donchian universe refresh exposed that the five-minute setting is a post-cycle sleep, sector hydration can block startup, and open positions are not prioritized ahead of flat-symbol cache work | Inventory and measure first; write and approve the cache/fetch scheduling design before implementing fixes |
| 5 | `11.72` shared watchlist fundamentals parsing | **Audit complete; proposal awaiting approval.** ISRG verifies the net-income alias gap, and the audit also found inconsistent unknown handling in the standalone review, SMA scanner, and legacy RSI builder. Frozen-window impact would add ISRG around Donchian rank 84; the stale RSI report also omits now-resolved BRK.B around rank 42 | Approve [`docs/watchlist_fundamentals_audit.md`](docs/watchlist_fundamentals_audit.md), then implement the shared fact/provenance contract and consistent fail-closed consumer reasons before any configured-universe change or Donchian recycle |
| 6 | `11.62` portfolio heat ceiling | Deferred while `11.60` has only two would-block events | Collect more sleeve-heat evidence before designing a portfolio-wide ceiling |
| 7 | RSI watchlist refresh | ✅ Paper exercised: the recycled bot reports 50 ranked names plus protected ABNB/CCK, and both protected positions retain ownership and stops | Remove ABNB/CCK only after each is flat and terminal; otherwise continue ordinary paper evidence collection |

Evidence still collecting: slippage calibration **2/10**; RSI3 **9 entries / 6 completed exits / 3 open**; credit-spread bounded entry walk **4/~20 attempts**; Donchian heat-cap observation **2 would-block events**; leveraged trend **4 open entries / 0 exits**. `11.61a` has one RSI contention group; offline replay finds both refused candidates filled and still open, so there are no resolved counterfactual outcomes yet. `11.41a` and `11.54a` remain event-gated. VPS and live-flip tasks remain deferred by operator decision.
...
| Trustworthy strategy graduation report | Lifecycle-first reporting groups partial exits and MLEG legs into reconciled outcomes. Forward daily marks use durable realized P&L plus broker unrealized P&L; a versioned regulatory model estimates costs without double-counting fill slippage. Unknown, missing, and unmodelable values stay unavailable. | Deploy and review the first complete cohort marks and cost-adjusted reports. Then define evidence-sufficiency rules and bind explicit operator approvals to preflight in a separate PR. |
...
| `11.23` RSI3 simplification paper-watch | 🔄 **9 entries / 6 completed exits / 3 open since the reset.** Completed outcomes are 3 wins / 3 losses and about -$101 total. | Keep collecting; six outcomes are too few to judge expectancy, drawdown, or clustered dip-buying. |
| RSI 50-candidate static-pool refresh | ✅ **PROMOTED 2026-09-09.** Membership uses only durable price, dollar-liquidity, size, and affirmative-solvency gates. Temporary technical state and historical RSI outcomes are reference-only. Sector concentration is accepted without caps or reranking. The active list is the fresh SIP report's 50 names plus protected open ABNB/CCK; `GOOGL` is rejected and eligible `GOOG` is preserved. Risk sizing binds for 38/50, with 12 safely cap-clipped. | New config-hash evidence cohort begins on recycle. Remove ABNB/CCK only after each is flat and terminal; continue ordinary paper evidence collection. |
| Donchian 100-candidate durable-pool refresh | ✅ **PROMOTED 2026-09-19.** Replaces the manually themed 52-name universe with the delayed-SIP selector's first 100 durable companies by dollar liquidity, plus temporary lifecycle-preservation members from the prior list. Price, liquidity, size, and affirmative solvency determine membership; trend state and historical breakout results are diagnostic only. The 50/100/200 comparison selected 100 as the first broad paper cohort: 896 trailing raw breakouts vs 479 at 50 without the 200-name pool's 315-evaluation cycle load and 79 raw over-capacity days. | New config-hash evidence cohort begins on recycle. Remove lifecycle-preservation members only after they are flat and terminal. Track contention, warmed cycle duration, candidate starvation, and 56/100 conservative risk-target clips; do not tune ranking or risk from the same snapshot. See [`docs/donchian-watchlist-selection.md`](docs/donchian-watchlist-selection.md). |
| `11.72` Shared watchlist profitability parsing and solvency provenance | **AUDITED — proposal ready, implementation awaiting approval.** The verified ISRG miss comes from requiring Yahoo's exact `Net Income` row even though `Net Income Common Stockholders` is present and positive. The broader audit found that RSI and Donchian safely reject unknown solvency but conflate it with failure, the standalone report calls missing required facts `GOOD FIT`, SMA can therefore admit unknown required fundamentals, and the historical RSI builder passes `solvency_ok=None`. Frozen-window analysis puts corrected ISRG around Donchian rank 84; separately, the RSI report predates the BRK.B provider-alias fix and would place it around rank 42. | Approve and implement [`docs/watchlist_fundamentals_audit.md`](docs/watchlist_fundamentals_audit.md): exact `Net Income` first, narrow `Net Income Common Stockholders` fallback with source provenance, explicit unknown/fail reasons, an `UNKNOWN` standalone verdict, and affirmative-solvency requirements for every consumer. Regenerate sanitized and private RSI/Donchian reports, separate parser effects from provider/market drift, preserve open-position coverage, and require explicit operator approval before changing either configured universe. Complete before recycling the Donchian cohort introduced by #160. |
| `11.71` Runtime cache, startup hydration, and cycle-latency audit | **OPEN — audit and design first; no piecemeal fixes authorized.** The five-minute engine setting sleeps after a completed sweep rather than imposing a deadline, so a seven-minute cycle produces roughly twelve minutes between starts. Sector metadata hydration runs before the stream and engine, declares but does not enforce a per-symbol timeout, and persists successful results only after the full loop. Bar caches are per-symbol and durable, but cold or retry-heavy work can delay later symbols; open positions are not explicitly processed before flat candidates. The larger Donchian pool makes these bot-wide contracts worth reviewing together rather than tuning one scanner. | Phase A: inventory every startup and cycle cache/fetch path (sector metadata, OHLCV/feed-aware bars, regime/sector ETF data, IV proxies, quotes, and relevant in-memory TTLs); document source of truth, cache key/provenance, freshness, invalidation, retry/timeout/backoff, partial-progress durability, atomicity/corruption recovery, restart behavior, and failure mode. Measure cold, warm, degraded-network, and interrupted-start baselines on the current full slot universe, including p50/p95/max cycle duration and position-to-evaluation latency. Produce and approve a design covering bounded startup, incremental persistence, explicit prewarming, overrun observability, non-overlapping scheduling semantics, and safety-first ordering of broker reconciliation/open-position exits ahead of flat-symbol entry scans. Phase B must be a separate reviewed implementation with deterministic failure-injection tests, cache migration/rollback notes, full-suite validation, and an off-hours paper cold-start/restart exercise. Do not increase the Donchian pool beyond 100 or treat a collection of local timeout patches as completion. |
...
| `11.63` Credit-spread entry bounded walk | 🔄 **4 post-change attempts: 1 fill, 3 cancels.** The target is about 20 attempts; the pre-change baseline was 13 fills in 49 attempts (27%). | Keep collecting clamp and fill evidence. Do not tune the credit floor from four attempts. |
...
| `11.61` Entry-candidate ranking | 🔄 **RSI SHADOW RESOLUTION AVAILABLE OFFLINE; ranking off.** One real group produced four candidates for two slots. Replay follows the actual GTC-limit contract and finds both refused candidates filled and still open; dollar P&L remains unknown because no quantity was approved. Other strategies remain unsupported until they produce a real contention case. | Refresh open shadows after later sessions. Require repeated resolved RSI contention before a separately reviewed `11.61b` ranking rule. Never compare scores across strategies. See [`docs/entry_candidate_observation.md`](docs/entry_candidate_observation.md). |
| RSI GTC limit stop anchoring | Ordinary whole-share GTC LIMIT + OTO entries retain the reference-anchored stop, while capped and fractional equity entries re-anchor protection to the fill. A gap-down fill therefore changes the intended ATR breathing room. | Fix production in a separate focused PR. Preserve historical truth by versioning `stop_anchor` in the `11.61` replay contract. |
