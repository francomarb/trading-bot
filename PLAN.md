...
| 10.F3a | **SMA edge filter** — implement `strategies/filters/sma_crossover.py` with two entry gates: (1) **market-trend confirmation** — SPY close > SPY SMA200; blocks new SMA entries in broad market downtrends where trend-following degrades; (2) **earnings-blackout veto** — block new SMA entries in the N calendar days surrounding a symbol's earnings date; SMA crossover exits are too slow to protect against overnight gaps. Shared calendar/event helpers live in `strategies/filters/common.py`. Wire into `SMACrossover` via the existing `edge_filter` hook. Exits never blocked. Unit tests: market-trend gate blocks entries when SPY below 200SMA; earnings veto blocks entries within the blackout window; exits fire regardless; filter passes when both conditions are clear. Promoted from Phase 11 item 11.20 — required before SMA goes live. | Medium (~60 lines) | 🔴 | ⬜ |
| 10.F3b | **RSI edge filter** — implement `strategies/filters/rsi_reversion.py` with Tier 1 gates (from `docs/RSI-edge-filter.md`): (1) SPY close > SPY SMA200; (2) SPY close > SPY SMA50; (3) stock close > stock SMA50. All three must be true for a new RSI entry. Exits never blocked. Logs `RSI_FILTER_ALLOWED` or `RSI_FILTER_BLOCKED` with reason for every raw signal (required observability). Wire into `RSIReversion` via the existing `edge_filter` hook. Unit tests: each gate blocks independently; all-clear allows entry; exit fires when blocked. Required before RSI paper activation in 10.F4. | Low (~40 lines) | 🔴 | ⬜ |
