...
| 4 | `11.71` runtime cache and cycle-latency audit | **AUDIT COMPLETE; DESIGN PROPOSED — awaiting approval before implementation.** Local cache reads are healthy and fast; the material risks are pre-safety sector hydration, redundant completed-daily-bar API refreshes, nested SDK/local retries, non-atomic/memory-only caches, fixed-delay cadence, non-prioritized signal exits, and synchronous order waits. See `docs/runtime_cache_cycle_audit.md` | Review the proposed PR A/B/C sequence. Reuse Alpaca-native multi-symbol requests and established atomic/TTL/locking primitives; begin with telemetry plus safety ordering |
| 5 | `11.72` shared watchlist fundamentals parsing | ✅ **COMPLETE.** PR #161 merged, main was pulled, and the bot was recycled via `./recycle_bot.sh`. Startup ownership/stops and the 100 Donchian / 50 RSI ranked opportunity counts were verified; lifecycle-protection members remain appended outside those ranked pools | Continue quarterly/stability-triggered refreshes rather than reacting to one-scan liquidity churn |
...
| `11.72` Shared watchlist profitability parsing and solvency provenance | ✅ **COMPLETE.** The shared parser preserves exact `Net Income` precedence, adds the narrow `Net Income Common Stockholders` fallback and source provenance, and exposes explicit solvency reasons. RSI, Donchian, SMA, the standalone report, and the historical RSI builder now fail closed consistently. ISRG replaced LIN in the Donchian ranked 100; BRK.B replaced BAC in the RSI ranked 50 on the frozen promotion window; required lifecycle protection remains appended outside the ranked pool. PR #161 merged, main was pulled, and the post-recycle ownership, stops, and ranked counts were verified. | Continue quarterly/stability-triggered refreshes rather than one-scan churn. |
| `11.71` Runtime cache, startup hydration, and cycle-latency audit | **PHASE A AUDIT COMPLETE; DESIGN PROPOSED — no runtime fixes applied.** The audit found healthy/fast local Parquet reads but unsafe pre-engine sector hydration, per-cycle re-fetching of completed daily inputs, unbounded Yahoo paths, non-atomic/memory-only caches, fixed-delay scheduling, no explicit owned-position-first signal pass, and execution waits that dominate several worst tails. The deployed 217-evaluation universe has not yet completed a market-open cycle; 34 required main daily series remain cold. Full evidence, inventory, failure semantics, metrics, and design are in `docs/runtime_cache_cycle_audit.md`. | Approve the staged design. PR A: telemetry plus safety bootstrap/owned-position ordering. PR B: atomic/versioned caches and bounded completed-bar prewarming. PR C: fixed-rate skip-missed-tick cadence and redundant-request removal. Treat asynchronous execution settlement as a separate decision after phase telemetry. Each PR requires deterministic failure injection, full-suite validation, rollback notes, and off-hours paper verification. Keep Donchian ranked size at 100. |
