...
| 10.F4 | **RSI paper activation slot** — add `RSIReversion` to the paper engine with `settings.RSI_WATCHLIST`, RSI edge filter (10.F3b) wired in, strategy-specific allocation, limit-order behavior, and attribution verified. Do not enable live. | Medium | 🔴 | ⬜ |
...
| 10.G6 | **Fractional share sizing** — `FRACTIONAL_ENABLED` flag in `config/settings.py`. Replace the current `floor(notional_budget / price)` with `round(notional_budget / price, 2)` so every position targets the correct notional exposure regardless of share price. Two benefits: (1) **rounds-to-0 fix** — stocks priced above the notional budget (e.g. AVGO, NVDA on a small account) get a trade instead of being skipped; (2) **sizing accuracy** — $300–600 stocks where whole-share rounding creates a 20–50% notional distortion are sized correctly. Alpaca fractional orders are `DAY`-only — no OTO stop leg can be attached. Engine monitors price each cycle and issues a fresh `DAY` stop order at session open until the position closes, covering the missing broker stop. Importance is highest at account launch and decreases naturally as the account grows (larger notional budget means whole-share rounding error shrinks). `FRACTIONAL_ENABLED=False` reverts to `floor()` and skips rather than fractions. Unit tests: fractional qty computed correctly; whole-share path unchanged when `FRACTIONAL_ENABLED=False`; engine stop monitor fires on next cycle after fractional fill; `MIN_NOTIONAL` rejects trades that are still too small after fractional sizing. | Medium (~80 lines) | 🟢 | ⬜ |
...
| 11.24 | **Sentiment overlay — social media and political event signals.** Market-moving sentiment does not come only from price action: a single Trump post about tariffs can reprice entire sectors within minutes; Reddit/WSB coordinated flows can force short squeezes; earnings rumor threads move stocks before any technical signal fires. This item adds a `SentimentSource` abstraction (parallel to `WatchlistSource`) that emits a per-symbol sentiment score or a market-wide risk-on/risk-off flag on each engine cycle. **Integration point:** the existing `edge_filter` hook in `BaseStrategy` is the natural home — a `SentimentEdgeFilter` can veto an otherwise valid SMA or RSI entry with zero changes to the engine or risk layer. Alternatively, a market-wide negative-sentiment flag can be fed to the `RegimeDetector` as an override tier above BEAR, immediately blocking all new entries regardless of technical state. **Data sources to evaluate:** (1) Alpaca News API — already available to Alpaca subscribers, provides news headlines with sentiment scores (positive/negative/neutral) per symbol and for the market; lowest-friction starting point. (2) StockTwits streaming API — symbol-tagged posts with a built-in bullish/bearish label; free tier available. (3) Reddit `pushshift` / official API — `r/wallstreetbets` mention count and upvote velocity as a retail-frenzy signal (WSB squeeze events: GME 2021, AMC, BBBY). (4) X (Twitter) API — real-time political figure post monitoring; high-value for macro risk-off events (tariff announcements, Fed policy signals from influential accounts). (5) Polygon.io / Benzinga news feed — paid, higher-quality, lower-latency than free tiers. **Key design considerations:** (a) Sentiment must be treated as a *veto gate* (block entries on strong negative) not a *signal generator* (do not initiate trades on positive sentiment alone — that is a different, higher-risk strategy class). (b) Political event monitoring (Trump posts, Fed chair speeches) is macro-level and should suppress all strategies simultaneously — wire it into the regime layer, not per-strategy filters. (c) Social retail frenzy (WSB mention spike) is symbol-level and bidirectional — a WSB spike into an RSI oversold name may actually confirm a reversion setup; a spike into an SMA trend name may indicate unsustainable momentum. Strategy-specific handling required. (d) Latency matters: a tariff post can move a sector 5% in under 60 seconds; a 5-minute polling cycle is too slow for political event response. A dedicated real-time webhook/stream listener running alongside the engine is needed for political events specifically. (e) Backtesting sentiment is hard — historical tweet/post data is expensive and sparse; validate on paper before relying on it as a veto gate. | ⬜ |
