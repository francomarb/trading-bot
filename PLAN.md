...
| 4 | `11.71` runtime cache and cycle-latency audit | **AUDIT COMPLETE; NARROW IMPLEMENTATION PROPOSED.** Review confirmed one immediate safety defect (unbounded sector hydration before broker safety work), an approximately 11-minute nested-retry upper bound, and a burst near the Basic plan's 200 market-data calls/minute. Normal cache/cycle evidence does not justify pre-approving the broader coordinator/scheduler/locking design. See `docs/runtime_cache_cycle_audit.md` | Next PR only: reorder/bound/incrementally persist sector hydration; remove nested/final-sleep retries; quarantine corrupt Parquet; add phase, actual-request, retry, order-wait, start-lag, and position-latency telemetry. Let events authorize batching or other follow-ups |
...
| `11.71` Runtime cache, startup hydration, and cycle-latency audit | **PHASE A AUDIT COMPLETE; REVIEW-TRIMMED NEXT PR — no runtime fixes applied.** The audit/review confirmed unsafe pre-engine sector hydration, an approximately 11-minute single-symbol nested-retry upper bound, unguarded Parquet reads, and a healthy-cycle burst already near the Basic plan's 200 market-data calls/minute. Execution waits dominate several worst tails and will not be fixed by cache work. The 217-evaluation universe still lacks a clean warmed market-open baseline; 34 required main daily series remain cold. See `docs/runtime_cache_cycle_audit.md`. | Next PR only: safety-first bounded/incremental sector hydration, one bounded read-retry policy with no final sleep, corrupt-Parquet quarantine, and phase/request/retry/order-wait/start-lag/position telemetry. Use event gates—not a five-session pause—to authorize native batching, completed-session refresh, position ordering, cadence changes, locks, or asynchronous settlement. Keep Donchian ranked size at 100. |
