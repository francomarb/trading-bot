...
| 10.F3b | **RSI edge filter** — `strategies/filters/rsi_reversion.py`. Three entry gates: (1) **SPY > 200 SMA**; (2) **SPY > 50 SMA** — combined dual-SMA macro gate; (3) **earnings blackout (3 days before / 1 day after)** — binary events work against mean-reversion. Stock 50-day SMA gate intentionally excluded: oversold RSI stocks are typically below their 50 SMA — filtering on that condition removes exactly the trades the strategy is designed to take. Observability: `RSI_FILTER_ALLOWED` / `RSI_FILTER_BLOCKED` logged per signal with specific reasons. | Low | 🔴 | ✅ (2026-04-25) |
