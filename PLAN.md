...
| Donchian trailing broker stop | **Parked pending SIP re-test 2026-06-07** (originally closed 2026-06-06; revised three times after reviewer findings). Question: should the broker-side ATR stop trail the trade up, instead of sitting at `entry − 2×ATR` forever? Worry: a gap-down past the rising 15-day-low exits at the original stop and gives back all accrued profit. **Coverage caveat — important**: the IEX-only investigation tested 2021-04-01 → 2024-12-31, which does **not** include the 2018 Q4 vol shock or the 2020 COVID crash the PLAN concern originally cited. Subsequent SIP probing (verified 2026-06-07) shows SIP individual-stock coverage extends back to **2016-01-04** for AAPL/MSFT/NVDA and most ai_bigtech mega-caps, so those regimes **are** reachable on the basic Alpaca tier via delayed SIP. Re-testing on SIP is the blocking step before this row can move to fully closed; tracked by PR #50 (feed-aware cache + SIP-default backtest infrastructure). **Findings on IEX-reachable window (R2 numbers, 207 combined trades, ai_bigtech, production gates ON):** Donchian-low trail is a wash vs static — returns within 0.1 pp, Sharpe identical at 0.18, MaxDD within 0.1 pp. Chandelier helps marginally in chop but gives back 0.9 pp in the 2023-24 AI rally because it clips trending winners. Gap-through exits do happen (7-14% of static-stop trades) but the strategy's signal exit catches most trend failures on the next-day close — within the IEX-reachable regimes. Whether the conclusion survives on the catastrophic-gap regimes (Q4 2018, March 2020) is **the question still open**. Full writeup: [docs/donchian_trail_investigation.md](docs/donchian_trail_investigation.md). Simulator: [backtest/donchian_trail_sim.py](backtest/donchian_trail_sim.py). Reproduce: `venv/bin/python scripts/donchian_trail_compare.py`. Re-open formally with the SIP re-test once PR #50 lands, or with live paper evidence of a meaningful giveback event. |
