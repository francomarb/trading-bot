...
| ~~`11.73` Donchian parameter rebaseline on the durable pool~~ | ✅ **COMPLETE — retain 30/15.** The final production-parity correction sizes STOP_LIMIT orders from the worst permitted limit down to the pre-fill reference-anchored stop, then models the fill-anchored 2 ATR protection. It also includes the allocator's pre-sizing $100 availability floor. Stitched held-out results were 20/10 +14.3% / 0.97 Sharpe, 30/10 +18.4% / 1.01, 30/15 +15.7% / 0.71, and 55/20 +15.5% / 0.93. The expanding selector chose 30/10 in all five folds, but 20/10, 30/10, and 55/20 all failed the pre-registered remove-best-year test; 20/10 and 55/20 also failed remove-best-symbol. This is fixed-current-cohort robustness evidence, not authority over paper outcomes. See [`docs/donchian_parameter_rebaseline.md`](docs/donchian_parameter_rebaseline.md). | Keep 30/15 and collect the frozen universe's paper cohort. Reopen parameters only with materially new forward evidence and a new pre-registration; do not salvage a challenger by changing this test after the result. |
| `11.74` Donchian deployment validation | 🔄 **HISTORICAL COMPARISON COMPLETE; FORWARD GATE OPEN — plausible core signal, unproven deployment, strongly adverse initial regime.** Evidence defects are repaired and the corrected 100-name configuration is frozen. The 4R shared-capital comparison found classic 20/10 improved Sharpe/drawdown versus current close-based 30/15 on both the old 32 and durable 100, while classic 55/20 was stronger on aggregate return but more contributor-concentrated. The 4R cap refused classic pending-entry bursts but did not bind current close-based 30/15 inside the isolated baseline sleeve. These already inspected fixed cohorts are diagnostic only. See [`docs/donchian_deployment_validation.md`](docs/donchian_deployment_validation.md). | Keep production/paper at close-based 30/15. Continue exact-config paper evidence until at least 25 trusted exits **and** enough independent entry clusters resolve; symbol count alone is not evidence count. Only then reconsider a separately pre-registered paper implementation experiment or graduation review. |
